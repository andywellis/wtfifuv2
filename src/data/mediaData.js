// ============================================================
// WTFIFU — Curated Educational Media Data
// ============================================================
// Each entry = one specific episode, video, or article.
// NOT channels or series — individual pieces of content only.
//
// VETTING CRITERIA:
//   1. Presenter has peer-reviewed publications in topic area
//      OR holds/held university faculty appointment in discipline
//      OR has documented, recognized clinical expertise
//   2. Content freely accessible (no paywall)
//   3. No undisclosed industry sponsorship
//   4. Does not contradict current SR/MA evidence without
//      acknowledging controversy
//
// TAB_CONFIG for "learn" specialty:
//   summary  → "Summary"     — what the episode covers and why it matters
//   takeaways → "Key Takeaways" — specific clinical pearls, protocols, decision points
//   context  → "Context & Credentials" — presenter background, referenced literature
// ============================================================

// --- CITATIONS ---
export const MEDIA_CIT = {
  ...
  "gulati-2016-rbb-part1": {
    pmid: null,
    title: "Resin Bonded Bridges — the problem or the solution? Part 1: Assessment and Design",
    authors: "Gulati J",
...
// --- MEDIA ENTRY FORMAT ---
// format:   "podcast" | "video" | "lecture" | "article" | "case-report"
// platform: where it lives (YouTube, Spotify, Apple Podcasts, website, etc.)
// url:      direct link to the free content
// duration: approximate runtime in minutes
// presenter: { name, credentials, affiliation }
// tags:     array of content-type tags for filtering
// mapTo:    array of WTFIFU specialty/subcategory IDs this content enriches

// --- RESTORATIVE DENTISTRY MEDIA ---
export const RESTORATIVE_MEDIA = [
  {
    ...
    cat: "Restorative — Adhesive Protocols & Material Selection",
    ...
    presenter: {
      ...
      credentials: "DMD, PhD — University of Geneva. Associate Professor, USC Ostrow School of Dentistry. Director, Center for Esthetic Dentistry. Author of 100+ peer-reviewed articles.",
...

  {
    ...
    name: "Which Ceramic Should I Use? — Comprehensive Material Selection with Ed McLaren",
    source: "Protrusive Dental Podcast — PDP117",
    cat: "Restorative — Ceramic Materials & Classification",
...

  {
    ...
    name: "Extreme Bonding Exposed — Deep Margin Elevation & Adhesive Principles with David Gerdolle",
    source: "Protrusive Dental Podcast — PDP077",
    cat: "Restorative — Bonding Science & Technique",
...

  {
    ...
    name: "Ceramic Onlays — Full Protocol from Prep to Bonding with Nik Sethi",
    source: "Protrusive Dental Podcast — PDP059",
    cat: "Restorative — Indirect Restorations & Cementation",
    ...
    context: "Nik Sethi — UK clinician and co-founder of Elevate Dentistry, which runs hands-on adhesive/indirect courses. Recognized educator in the UK dental community. Mentored by Sanjay Hybrid (prosthodontist). Not primarily a published researcher but a respected clinical educator whose protocols align with current SR/MA evidence.\\n\\nIndustry disclosure: Sethi is affiliated with Elevate Dentistry courses (mentioned in episode).",
...
// --- CLASSIFIER ---
export function mediaGetGroup(cat) {
  if (!cat) return "Uncategorized";
  const c = cat.toLowerCase();
  if (c.includes("adhesive protocol") || c.includes("material selection")) return "Adhesive Protocols & Material Selection";
  if (c.includes("bonding science") || c.includes("bonding technique")) return "Bonding Science & Technique";
  if (c.includes("ceramic material") || c.includes("ceramic classification")) return "Ceramic Materials & Classification";
  ...
  if (c.includes("surface treatment") || c.includes("fiber reinforcement")) return "Surface Treatments & Fiber Reinforcement";
  return "Uncategorized";
}

// --- COMBINED EXPORTS ---
export const ALL_MEDIA = [...RESTORATIVE_MEDIA];
