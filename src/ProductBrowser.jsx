...
import { PROSTH_MATERIALS, PROVISIONALS } from "./data/prosthData.js";
...
export default function ProductBrowser({ specialty, onGoHome }) {
  ...
  const allItems = useMemo(() => {
    ...
    // Filter to only groups relevant to this specialty
    return all.filter(x => specGroups.includes(getGroup(x.cat)));
  }, [specGroups]);

  // Available groups within this specialty (for filter tabs)
  const availableGroups = useMemo(() => {
    const gs = new Set();
    allItems.forEach(x => gs.add(getGroup(x.cat)));
    return GROUPS.filter(g => gs.has(g));
  }, [allItems]);

  const filtered = useMemo(() => {
    let items = allItems;
    if (filter !== "All") items = items.filter(x => getGroup(x.cat) === filter);
    if (search.trim()) {
      const s = search.toLowerCase();
      items = items.filter(x => x.name.toLowerCase().includes(s) || x.mfr.toLowerCase().includes(s) || x.cat.toLowerCase().includes(s) || (x.composition || "").toLowerCase().includes(s) || (x.cureType || "").toLowerCase().includes(s) || (x.gen || "").toLowerCase().includes(s));
    }
    return items;
  ...
  const groups = useMemo(() => {
    const g = {};
    filtered.forEach(m => { const grp = getGroup(m.cat); if (!g[grp]) g[grp] = []; g[grp].push(m); });
    return g;
  }, [filtered]);
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px" }}>
      ...
      {availableGroups.filter(g => groups[g]).map(gn => (
        ...
        <div style={{ textAlign: "center", padding: "24px 0 10px", color: "#1e293b", fontSize: 9, letterSpacing: 1.5 }}>
          EVIDENCE-BASED · IFU CONFLICTS NOTED · 2-BOTTLE SILANE PREFERRED · VERIFY WITH CURRENT LITERATURE
        </div>
    </div>
  );
}
